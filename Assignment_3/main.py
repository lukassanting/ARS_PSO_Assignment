...
def evolution(
    ...
    for i in range(population_size):
        NN_coords = networks[i].prop_forward(input_sensors=np.array([XY_coords[0][i], XY_coords[1][i]]))
        if verbose: print(f"Old coordinates = {XY_coords[0][i]}, {XY_coords[1][i]}")
        if verbose: print(f"New coordinates = {NN_coords[0]}, {NN_coords[1]}")
...
evolution(network_layers=(2,12,4,2), bias=(False,True,True,False), population_size=10, verbose=True)
